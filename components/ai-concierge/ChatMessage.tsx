...
import { ArrowRight, Bot, Car, Clock3, Gauge, Sparkles, User } from "lucide-react";
import type { ReactNode } from "react";
import type { ChatMessage as ChatMessageType, VehicleCardData } from "@/lib/ai-concierge/engine";
    ...
    >
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        <div className="absolute left-3 top-3 flex items-center gap-1.5 rounded-full border border-white/10 bg-black/60 px-2.5 py-1.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-white backdrop-blur-md">
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              vehicle.status.toLowerCase() === "available" ? "bg-emerald-400" : "bg-amber-400"
            }`}
          />
          {vehicle.status}
        </div>
        ...
        <div className="absolute bottom-3 left-3 right-3">
          ...
          <h4 className="mt-1 text-base font-semibold tracking-[-0.02em] text-white">{vehicle.name}</h4>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-[9px] font-medium uppercase tracking-[0.14em] text-white/40">Starting from</p>
            <p className="mt-1 text-xl font-semibold text-white">{vehicle.priceLabel}</p>
            {vehicle.emiLabel ? (
              <p className="mt-0.5 text-[9px] text-[#CBA36B]/80">{vehicle.emiLabel}</p>
            ) : null}
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          <Stat icon={<Clock3 size={12} />} value={vehicle.mileageLabel} label="Mileage" />
          <Stat icon={<Gauge size={12} />} value={vehicle.transmission} label="Gearbox" />
          <Stat icon={<Car size={12} />} value={vehicle.fuelType} label="Fuel" />
        </div>

        <button
          type="button"
          onClick={() => onAction(`Tell me more about the ${vehicle.name}`)}
...
function Stat({ icon, value, label }: { icon: ReactNode; value: string; label: string }) {
  return (
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.025] px-2.5 py-2.5">
      <div className="flex items-center gap-1.5 text-[#CBA36B]">{icon}</div>
      <p className="mt-1 truncate text-[10px] font-medium text-white/75">{value}</p>
      <p className="mt-0.5 text-[8px] uppercase tracking-[0.1em] text-white/35">{label}</p>
    </div>
  );
}
      ...
      <div className={`flex max-w-[90%] flex-col ${isUser ? "items-end" : "items-start"}`}>
        <div className="mb-1.5 flex items-center gap-2 px-1">
          {!isUser ? (
            ...
            <>
              <span className="text-[8px] font-semibold uppercase tracking-[0.14em] text-white/35">You</span>
        ...
        {!isUser && message.vehicle ? (
          <div className="w-full">
            <VehicleCard vehicle={message.vehicle} onAction={onSuggestion} />
          </div>
        ) : null}
        ...
        <span className="mt-1.5 px-1 text-[8px] text-white/25">{message.timestamp}</span>
      </div>
    </motion.div>
  );
}